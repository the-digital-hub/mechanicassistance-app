...
type AvailabilityFormProps = {
  /**
   * Read once, on mount. A caller hydrating from the API renders the form only
   * after the data arrived, rather than pushing new values into a mounted form.
   */
  initialValue?: Partial<AvailabilityValue>;
  /** Centre of the service area; the placeholder is used while it is null. */
  baseLocation?: BaseLocation | null;
  onEditLocation: () => void;
  submitLabel: string;
  submitting?: boolean;
  /** Only called with a value that passed validation. */
  onSubmit: (value: AvailabilityValue) => void | Promise<void>;
};

/**
 * Weekly schedule + service area. Shared by the signup step
 * (`app/setup/availability.tsx`) and the mechanic's profile
 * (`app/(tabs)/availability.tsx`); each decides where the value goes.
 */
export function AvailabilityForm({
  initialValue,
  baseLocation: baseLocationProp,
  onEditLocation,
  submitLabel,
  submitting = false,
  onSubmit,
  ...
  const { t } = useTranslation();
  const baseLocation = baseLocationProp ?? BASE_LOCATION;

  const [selectedDays, setSelectedDays] = useState<string[]>(
    initialValue?.selectedDays ?? DEFAULT_DAYS,
  );
  const [applySameTime, setApplySameTime] = useState(
    initialValue?.applySameTime ?? true,
  );
  const [startTime, setStartTime] = useState(
    initialValue?.startTime ?? DEFAULT_START_TIME,
  );
  const [endTime, setEndTime] = useState(
    initialValue?.endTime ?? DEFAULT_END_TIME,
  );
  ...
  const [perDay, setPerDay] = useState<Record<string, DayHours>>(() =>
    Object.fromEntries(
      (initialValue?.schedule ?? []).map(({ day, startTime, endTime }) => [
        day,
        { startTime, endTime },
      ]),
    ),
  ...
  const [serviceRadius, setServiceRadius] = useState(
    initialValue?.serviceRadius ?? DEFAULT_SERVICE_RADIUS,
  );
   ...
   * back gesture is now off for the signup screen (the header's back arrow
   * stays), the thumb is clamped inside the track, and these handlers refuse to
   * hand the gesture over once the drag started.
   */
  ...
  const handleSave = async () => {
    ...
    // Week order, not tap order: the profile screen reads it back as-is.
